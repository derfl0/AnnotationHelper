...
class AnnotationhelperPlugin extends StudIPPlugin implements SystemPlugin {
    ...
    public function show_action() {
        if (Request::submitted('search')) {
            ...
            $stmt = DBManager::get()->prepare("SELECT username,CONCAT_WS(' ',vorname, nachname) as name FROM auth_user_md5 WHERE (username LIKE :search OR nachname LIKE :search ) AND " . get_vis_query() . " LIMIT 5");
